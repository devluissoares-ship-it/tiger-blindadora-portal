...
export default function AIAssistant({ cliente, isUserAdmin = false }: { cliente: any, isUserAdmin?: boolean }) {
  ...
  const enviarParaIA = async (textoPersonalizado?: string) => {
    ...
    if (!textoEnvio.trim()) return;

    setCarregando(true);
    try {
      const endpoint = isUserAdmin ? '/api/admin-ai' : '/api/chat-status';
    ...
    <div className="bg-[#0a0a0a] p-6 rounded-2xl border border-[#222] shadow-xl flex flex-col h-full">
        ...
        <div className="mb-4 flex gap-2 flex-wrap">
          {isEntrada ? (
            <button 
              ...
              disabled={carregando}
              className="text-[11px] bg-[#111] border border-[#222] hover:border-orange-500 text-gray-300 px-3 py-1.5 rounded-lg flex items-center gap-1.5 transition"
            >
      ...
      {!isUserAdmin && (
        <button 
          ...
          disabled={carregando}
          className="w-full bg-orange-500 text-black font-bold py-3 rounded-xl mb-4 hover:bg-white transition-all text-xs tracking-wider"
        >
      ...
      <div className="flex gap-2">
        ...
        <button 
          ...
          disabled={carregando}
          className={`p-3 rounded-xl transition disabled:opacity-50 flex items-center justify-center min-w-[48px] ${isUserAdmin ? 'bg-orange-500 hover:bg-orange-600 text-black' : 'bg-orange-500 hover:bg-white text-black'}`}
        >
